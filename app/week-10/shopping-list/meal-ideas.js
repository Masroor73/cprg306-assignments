...
export default function MealIdeas({ ingredient }) {
  ...
  // Expand meal by toggling
  const handleMealClick = async (mealId) => {
    if (mealId === selectedMealId) {
      // Clicking the same meal, collapses it
      setSelectedMealId(null);
      setMealDetails(null);
      return;
    }
    // Or else, details is fetched for the newly selected meal
    setSelectedMealId(mealId);
    const details = await fetchMealDetails(mealId);
    setMealDetails(details);
  };

  // Ingredients are extracted from meal details
        ...
        {meals.map((meal) => {
          ...
          return (
            <li
              ...
              className={`p-2 rounded cursor-pointer transition-colors bg-gray-700 hover:bg-orange-500
